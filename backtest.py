"""
v1.0 - Dual Moving Average Crossover Backtest Baseline
Implements core Golden/Death Cross logic with a fixed 5% hard stop-loss.
Establishes the statistical performance baseline for subsequent volatility
adjustments and asset allocation modules.
Date: May 2026
"""
...
def get_annual_trading_days(ticker):
    # Account for crypto trading 365 days vs equity 252 days to avoid Sharpe distortion
...
def run_backtest(ticker, short_window, long_window, stop_loss_pct=0.05, show_plot=True):
    """Bar-by-bar time-series backtesting engine with a fixed percentage hard stop-loss."""
    ...
    if raw_df.empty:
        print(f"Warning: No data returned for {ticker}. Skipping market.")
        return None
    ...
    commission = 0.001   # 0.1% round-trip friction and slippage estimate
    ...
    # Iterative loop enforced to completely eliminate look-ahead bias common in vectorization
    for i in range(1, len(df)):
        ...
        if position == 1:
            ...
            if drawdown_from_entry <= -stop_loss_pct:
                # Hard stop triggered. Assumes instantaneous execution; overnight gaps may yield worse slippage
                ...
                # Exit signal deferred by one bar to execute cleanly on confirmed daily close
    ...
    if show_plot:
        ...
        axes[0].plot(df.index, df['Strategy_Cum'], label=f'MA({short_window},{long_window})', color='steelblue')

        dd_series = df['Strategy_Cum'] / df['Strategy_Cum'].cummax() - 1.0
        dd_end    = dd_series.idxmin()
        dd_start  = df['Strategy_Cum'].loc[:dd_end].idxmax()
        axes[0].axvspan(dd_start, dd_end, color='red', alpha=0.15, label='Max Drawdown')

        axes[0].set_title(f"{ticker} | MA({short_window},{long_window}) | Ret: {total_return * 100:.1f}% | Sharpe: {sharpe:.2f}", fontsize=9)
...
if __name__ == "__main__":
    ...
    print("[Phase 1] Initializing in-sample parameter grid search on AAPL...")
    ...
    print("\n--- AAPL Grid Search Top 5 Profiles ---")
    ...
    print(f"\nOptimal Parameter Configuration: MA({best_short}, {best_long}) | Sharpe: {best_row['Sharpe']}")

    print(f"\n[Phase 1] Launching cross-market multi-asset stress test...")
    ...
    print("\n=== Cross-Market Strategy Summary Matrix ===")
