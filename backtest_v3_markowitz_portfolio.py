...
if __name__ == "__main__":
    ...
    # Annualizing parameters assuming linear return scaling and square-root time scaling for risk
    annual_returns = daily_returns.mean() * num_trading_days
    # TODO: sample covariance matrix is highly unstable here. consider a shrinkage estimator (e.g. Ledoit-Wolf) if optimal weights get too extreme.
    annual_covariance = daily_returns.cov() * num_trading_days

    # SLSQP constraints: weights must sum up to exactly 1.0 (fully invested)
    constraints = ({'type': 'eq', 'fun': lambda x: np.sum(x) - 1})
    # Long-only constraint: no short selling allowed (weights bound between 0 and 1)
